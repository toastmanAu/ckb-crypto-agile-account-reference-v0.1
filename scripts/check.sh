...
cargo build --locked --release --target riscv64imac-unknown-none-elf \
  ...
  -p verifier-mldsa-adapter \
  -p verifier-slhdsa-adapter
cargo test --locked -p ckb-account-protocol -p ckb-account-host -p ckb-account-tests -- --nocapture
cargo run --locked -p ckb-account-host --example export_vectors -- vectors/conformance-v1.bin
