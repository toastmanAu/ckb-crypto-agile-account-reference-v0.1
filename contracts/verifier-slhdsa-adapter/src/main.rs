...
use ckb_account_protocol::{
    parse_public_key_proof, VerifierRequest, ALG_SLHDSA, PROFILE_V1, SLH_DSA_SHA2_128S,
    VERIFIER_MAX_REQUEST_LEN, VERIFIER_REQUEST_HEADER_LEN,
};
...
fn verify(request: &[u8]) -> Result<(), i8> {
    ...
    let key =
        VerifyingKey::<Sha2_128s>::try_from(proof.public_key).map_err(|_| exit::MALFORMED_KEY)?;
    let signature =
        Signature::<Sha2_128s>::try_from(proof.signature).map_err(|_| exit::MALFORMED_PROOF)?;
